...
**user** - Создание, удаление, модификация пользователей.

**group** - Создание групп.

**authorized_key** - Добавление SSH-ключей пользователю.

**passwd** - Изменение пароля пользователя (хеш).

##### Сеть и загрузка файлов:

**get_url** - Скачивание файла из интернета (HTTP/HTTPS/FTP). Аналог `wget`/`curl`.

**uri** - Выполнение HTTP-запросов (GET, POST, PUT). Проверка API.

**wait_for** - Ждать появления порта, файла или процесса. Полезно для ожидания старта БД.

**iptables / ufw** - Управление фаерволами.

**hostname** - Изменение имени хоста.

##### Отладка и логика:

**debug** - Вывод сообщений или переменных на экран.

**set_fact** - Создание переменной "на лету" во время выполнения плейбука.

**assert** - Проверка условия. Если ложь — плейбук падает с ошибкой.

**fail** - Принудительно остановить выполнение с сообщением об ошибке.

**pause** - Остановить выполнение на время или до нажатия Enter (для ручного подтверждения).
