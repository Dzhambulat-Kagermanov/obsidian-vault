...
---
**Communities:** Специальные метки (теги), которые вы вешаете на маршруты, чтобы договориться с провайдером о действиях (например, «не транслировать дальше», «предпочесть этот путь»). 

Сам по себе атрибут Community **ничего не делает**. Он просто «путешествует» вместе с маршрутом. Магию совершает тот роутер, который **читает** эту метку и настроен реагировать на неё определенным образом. Обычно его записывают в формате `ASN:VALUE`.
...
Игнорирование конкретных well-known community меток:
```Router
