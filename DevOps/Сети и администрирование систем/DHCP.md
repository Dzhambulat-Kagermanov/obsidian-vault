...
```bash
apt-get update && apt-get install kea4
```
...
Параметр **"interfaces-config"** перечисляет список интерфейсов, на которых будет работать DHCP сервер, списки в JSON заключаются в квадратные скобки и значения перечисляются через запятую. **Параметр не обязательный**. Если мы хотим принимать запросы на двух интерфейсах, то список будет выглядеть следующим образом:
