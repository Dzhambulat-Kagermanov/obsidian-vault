...
> 
> - Механизм работает поверх [[1. apt| apt]], используя его для загрузки и установки.
> - Физическую распаковку и настройку выполняет [[2. dpkg| dpkg]].
> - Для выбора репозиториев используются те же метаданные, что и в [[3. Репозитории| репозиториях]].
> - Если пакет добавлен в blacklist, это работает аналогично [[4. pinning| apt pinning]] с приоритетом, запрещающим обновление.
...
>В production-среде с stateful-приложениями или кластерами без правильной оркестрации автоматическая перезагрузка может привести к простою. Лучше полагаться на [[3. Deployment| Rolling Updates]] или инструменты управления конфигурацией, а `Automatic-Reboot` оставлять в `"false"`.
...
>[!LINK] Связь с IaC
>Этот подход является частью [[1. Роли и Коллекции| создания ролей Ansible]] для обеспечения конфигурационной целостности (Configuration Drift prevention).
...
5. Каким образом `Package-Blacklist` в unattended-upgrades взаимодействует с механизмом [[4. pinning| apt pinning]], если для пакета задан приоритет?
