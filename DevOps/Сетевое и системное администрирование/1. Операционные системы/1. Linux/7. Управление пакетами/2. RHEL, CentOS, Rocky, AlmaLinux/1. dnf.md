...
> 
> - `dnf` является аналогом [[1. apt| apt]] в мире Red Hat.
> - Низкоуровневую установку `.rpm` файлов выполняет `rpm` (аналог [[2. dpkg| dpkg]]).
> - Управление источниками пакетов осуществляется через файлы `.repo`, что концептуально схоже с [[3. Репозитории| репозиториями APT]].
> - Скрипты сопровождения в RPM также являются [[4. Bash скрипты| bash-скриптами]].
...
> Проверка GPG-подписей (`gpgcheck=1`) критична для предотвращения установки модифицированных пакетов. Это часть [[2. Инфраструктура открытых ключей| PKI]].
