...
>Shell окружение строится на фундаменте [[1. Основы работы в терминале и навигация по ФС| основ работы в терминале]], использует [[2. Stdin, Stdout, Stderr, пайплайны, перенаправления| потоки ввода-вывода]] и является средой выполнения для [[4. Bash скрипты| bash-скриптов]]. Понимание структуры конфигурационных файлов требует знания [[1. etc структуры| структуры /etc]].
...
> - Скриншотах и логах терминала Всегда используйте менеджеры секретов ([[2. Динамические секреты| Vault]]) вместо передачи секретов в командной строке.
