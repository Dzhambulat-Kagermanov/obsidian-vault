...
>Работа со ссылками опирается на понимание [[1. Основы работы в терминале и навигация по ФС| структуры файловой системы]] и [[1. Принцип работы ОС| концепции inode]], так как именно inode является ключевым элементом в механизме жестких ссылок.
...
>Жесткие ссылки могут создаваться **только в пределах одной файловой системы**, так как inode уникальны только внутри FS. Символические ссылки этого ограничения не имеют.
...
> [!LINK] Связь с troubleshooting
> Диагностика проблем с путями и ссылками — часть [[2. Методология Troubleshooting| методологии диагностики]], особенно при анализе «файл не найден» при наличии файла.
...
>[!INFO] Контейнеры и ссылки
>В Docker/Kubernetes symlinks могут вести себя неочевидно при монтировании volumes. Всегда тестируйте разрешение путей внутри контейнера: `docker exec <container> readlink -f /path`.
